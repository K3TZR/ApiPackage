...
public enum FlexErrors: UInt32, CaseIterable, CustomStringConvertible {
  ...
  /// A textual explanation of the error code
...
/// Namespace enum for FlexError utility functions
public enum FlexError {
  
  /// Logs a Flex error based on the given error code and message text.
  ///
  /// - Parameters:
  ///   - errorCode: The error code from the API response in hexadecimal string format.
  ///   - msgText: The message text from the API response.
  public static func logError(errorCode: String, msgText: String) {
    guard let number = UInt32(errorCode, radix: 16) else {
      apiLog(.info, "FlexMsg: code <\(errorCode)> message <\(msgText)>")
      return
    }
    ...
    case 0x31000001...0x31000009:
      if let error = FlexErrors(rawValue: number) {
        apiLog(.warning, "FlexError: errorCode <\(errorCode)> description <\(error.description)> message <\(msgText)>")
      } else {
        apiLog(.warning, "FlexError: errorCode <\(errorCode)> description <Unknown error> message <\(msgText)>")
      }
      
    case 0x50000001...0x500000A3,
         0x500000BE...0x500000BE,
         0x50001000...0x50001017,
         0xE2000000,
         0xF3000001...0xF3000004:
      if let error = FlexErrors(rawValue: number) {
        apiLog(.error, "FlexError: errorCode <\(errorCode)> description <\(error.description)> message <\(msgText)>")
      } else {
        apiLog(.error, "FlexError: errorCode <\(errorCode)> description <Unknown error> message <\(msgText)>")
      }
  ...
  /// Returns a description string for the given error code.
  ///
  /// - Parameter errorCode: The error code from the API response in hexadecimal string format.
  /// - Returns: A human-readable error description or "Unknown error" if the code is not recognized.
  public static func description(_ errorCode: String) -> String {
    guard let number = UInt32(errorCode, radix: 16) else {
