...
//    // FIXME: how to update sample rate ???
  ...
  public func audioProcessor(_ payloadData: [UInt8], reducedBandwidth: Bool) {
    
    if reducedBandwidth {
      // Reduced Bandwidth DaxRxAudio
      Task { await _pcmProcessor.process(payloadData, reducedBandwidth: true) }

    } else {
      // Full Bandwidth DaxRxAudio
      Task { await _pcmProcessor.process(payloadData) }
    }
  }
}
